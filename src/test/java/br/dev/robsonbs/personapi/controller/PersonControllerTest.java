...
import static br.dev.robsonbs.personapi.utils.PhoneUtils.asJsonString;
...
public class PersonControllerTest {
  ...
  void testWhenPOSTIsCalledThenAPersonShouldBeCreated() throws Exception {
    ...
    when(personService.create(expectedPersonDTO))
            .thenReturn(expectedResponseMessage);
    mockMvc
            .perform(post(PEOPLE_API_URL_PATH)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(asJsonString(expectedPersonDTO)))
            .andExpect(jsonPath("$.message",
                    is(expectedResponseMessage.getMessage())))
            .andExpect(status().isCreated());
  }
  
  @Test
  void testWhenGETWithValidIsCalledThenAPersonShouldBeReturned() throws
          Exception {
    ...
    mockMvc
            .perform(get(PEOPLE_API_URL_PATH + "/" + expectedValidId)
                    .contentType(MediaType.APPLICATION_JSON))
  ...
  @Test
  void testWhenGETWithInvalidIsCalledThenAnErrorMessagenShouldBeReturned() throws
          Exception {
    var expectedValidId = 1L;
    PersonDTO expectedPersonDTO = createFakeDTO();
    expectedPersonDTO.setId(expectedValidId);
    
    when(personService.findById(expectedValidId))
  ...
  void testWhenGETIsCalledThenPeopleListShouldBeReturned() throws Exception {
    ...
    PersonDTO expectedPersonDTO = createFakeDTO();
    expectedPersonDTO.setId(expectedValidId);
    List<PersonDTO> expectedPeopleDTOList = Collections.singletonList(
            expectedPersonDTO);
    
    when(personService.listAll()).thenReturn(expectedPeopleDTOList);
    
    mockMvc
            .perform(get(PEOPLE_API_URL_PATH)
                    .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk())
  ...
  void testWhenPUTIsCalledThenAPersonShouldBeUpdated() throws Exception {
    ...
    when(personService.update(expectedValidId, expectedPersonDTO))
            .thenReturn(expectedResponseMessage);
    
    mockMvc
            .perform(put(PEOPLE_API_URL_PATH + "/" + expectedValidId)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(asJsonString(expectedPersonDTO)))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.message",
  ...
  void testWhenDELETEIsCalledThenAPersonShouldBeDeleted() throws Exception {
    ...
    mockMvc
            .perform(delete(PEOPLE_API_URL_PATH + "/" + expectedValidId)
                    .contentType(MediaType.APPLICATION_JSON))
